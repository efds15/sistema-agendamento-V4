...
async function ensureSchema() {
  try {
    ...
    console.warn('  ⚠️  Tabela notifications:', e.message)
  }
  ...
  try {
    ...
    console.log('  ✅ Colunas image_url OK')
  } catch {}
  ...
  try {
    ...
    if (n === 0) {
      ...
      console.log('  ✅ 9 aulas padrão inseridas')
    }
    console.log('  ✅ Tabela time_slots OK')
  } catch(e) { console.warn('  ⚠️  time_slots:', e.message) }

  try { await seed() } catch (e) { console.warn('Seed ignorado:', e.message) }
}
...
async function start() {
  ...
  app.listen(PORT, () => {
    console.log(`\n🎓  Agendamento API  →  http://localhost:${PORT}`)
